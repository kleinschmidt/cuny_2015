...
plot_category_bounds <- function(cat_bounds) {
  ggplot(cat_bounds,
         aes(x=bvotCond,
             y=boundary_vot,
             ymin=boundary_vot - 1.96*boundary_vot_se,
             ymax=boundary_vot + 1.96*boundary_vot_se,
             color=bvotCond, linetype=supCond,
             group=paste(shift, supCond))) +
    geom_pointrange(size=1.5, position=position_dodge(w=0.75)) +
...
```{r sup-unsup-mixed-stim-counts, fig.widht=12, fig.height=3}
...
dat %>%
  ...
  tally %>%
  ggplot(aes(x=vot, y=n, linetype=labeled)) +
  geom_bar(stat='identity', fill='white', size=2, color='black') +
  facet_grid(.~supCond) +
  scale_x_continuous('VOT (ms)', breaks=seq(-20, 80, by=20))
...
```{r expt1-cat-bounds, fig.width=4, fig.height=3}
...
```{r expt2-3-cat-bounds, fig.width=4, fig.height=3}
...
```{r expt4-cat-bounds, fig.width=6, fig.height=5}
